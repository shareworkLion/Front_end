...
function clickedMBTI(MBTI, placeNum) {
  if (newMBTI[placeNum] == null) {
    ...
    document.querySelector(`.mbti-${MBTI} img`).src = `../img/Button/MBTI_RAINBOW/MBTI_${MBTI}_Rainbow.png`
  }
}
function showNewMBTI() {
  if (newMBTI.length === 4) {
    console.log(newMBTI)
    document.querySelector('.currentMBTI').innerHTML = newMBTI.toString().replace(/,/g,"")
